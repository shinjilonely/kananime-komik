...
function ComicDetailView({ 
      ...
      try {
        ...
        if (res.ok) {
          ...
          setComic({
            ...
            genres: parsedGenres,
            chapters: (detail.chapters || detail.chapterList || []).map((ch: Record<string, unknown>) => ({
              chapter: ch.title || ch.chapter || ch.name || '',
              slug: ch.slug || ch.url || '',
              date: ch.date || '',
            })),
          });
  ...
  const imageUrl = getImageUrl(comic);
  const typeKey = comic.metadata?.type?.toLowerCase() || 'manga';
  const typeStyle = TYPE_STYLES[typeKey] || TYPE_STYLES.manga;
        ...
        <div className="flex-1 min-w-0 space-y-2">
          <div>
            <div className="flex flex-wrap gap-1 mb-1">
              <span className={`px-1.5 py-0.5 text-[9px] font-bold text-white rounded ${typeStyle.bg}`}>
                {(comic.metadata?.type?.toUpperCase() || 'MANGA')}
              </span>
              {comic.metadata?.status && comic.metadata.status !== 'Unknown' && (
                <span className={`px-1.5 py-0.5 text-[9px] font-medium rounded ${
                  comic.metadata.status === 'Ongoing' ? 'bg-green-500/20 text-green-400' : 'bg-muted text-muted-foreground'
                }`}>
